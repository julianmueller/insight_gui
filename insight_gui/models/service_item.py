...
from insight_gui.models.interface_item import ServiceInterfaceTypeItem
...
class ServiceItem(GObject.GObject):
    ...
    interface = GObject.Property(type=ServiceInterfaceTypeItem)
    # i just ignore that there could be multiple types of interfaces, because i think that it is stupid

    client_count = GObject.Property(type=int, default=0)
    server_count = GObject.Property(type=int, default=0)

    def __init__(self, *args, interface_full_name: str, **kwargs):
        super().__init__(*args, **kwargs)
        self.interface = ServiceInterfaceTypeItem(full_name=interface_full_name)
